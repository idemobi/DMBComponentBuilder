...
// DMBComponentBuilder.csproj BlockTitleBuilder.cs create at 2026/05/19
        ...
        ICanUseThemeVisibility
    {
            ...
            Shadow?       textShadow  = null)
        {
            if (!string.IsNullOrWhiteSpace(text))
                _subtitles.Add(new BlockTitleSubtitle { Text = text, Size = size, TextVariant = textVariant, TextShadow = textShadow });
            return this;
        }
        ...
        private void WriteTitle(TextWriter writer, HtmlEncoder encoder)
        {
            ...
            if (effectiveVariant.HasValue)
                tb = tb.SetTextVariant(effectiveVariant.Value);

            if (effectiveShadow.HasValue)
                tb = tb.SetTextShadow(effectiveShadow.Value);
